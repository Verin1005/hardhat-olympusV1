...
async function main() {
    ...
    var daiBond;
    try {
        daiBond = await DAIBond.deploy(time.address, dai.address, treasury.address, dao.address, zeroAddress);
    } catch (error) {
        console.log("daiBond error");
    }
    ...
    console.log(" ohm.balanceOf", String(await time.balanceOf(deployer.address)));
